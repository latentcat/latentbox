    ...
    >
      {props.data.map((item, index) => (
        <Link
          ...
          href={item.url}
          target="_blank"
          className="group relative rounded-xl overflow-hidden dark:bg-border/50 flex flex-col items-center justify-center p-6 pb-4"
        >
          ...
          <div
            className="absolute top-0 left-0 w-full h-full rounded-xl ring-1 ring-inset ring-border transition group-hover:ring-foreground" />
        </Link>
      ))}
    </div>
  )
